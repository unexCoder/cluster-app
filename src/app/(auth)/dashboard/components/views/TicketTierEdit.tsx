// views/TicketTierEdit.tsx
'use client'

import React, { useEffect, useState } from 'react'
import styles from './eventCreate.module.css'
import { fetchEventsAction } from '@/app/actions/events'
import { updateTicketTier } from '@/lib/api/ticketTier'

interface TicketTierEditProps {
  tierId: string
  initialData: any
...
export default function TicketTierEdit({ tierId, initialData, onNavigate }: TicketTierEditProps) {
  const [formData, setFormData] = useState<TicketTierForm>({
    event_id: '',
    name: '',
    description: '',
    price: '',
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      max_per_order: initialData.max_per_order?.toString() ?? '',
      sort_order:   initialData.sort_order?.toString() ?? '0',
      is_active:    Boolean(initialData.is_active),
      sales_start:  toDateTimeLocal(initialData.sales_start),
      sales_end:    toDateTimeLocal(initialData.sales_end),
      benefits:     parseBenefits(initialData.benefits),
    })
  }, [initialData])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: err instanceof Error ? err.message : 'Failed to update ticket tier' })
    } finally {
      setUpdating(false)
    }
  }
  ...
  return (
    <div className={styles.container}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '24px', fontWeight: '700' }}>Edit Ticket Tier</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {currentStep === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', fontWeight: '500' }}>Event *</label>
                <select
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #374151' }}>
            ...
            {currentStep === 1 ? (
              ...
              <button
                type="submit"
                disabled={updating}
                style={{ padding: '10px 24px', background: updating ? '#93c5fd' : '#3b82f6', color: 'white', border: 'none', borderRadius: '6px', cursor: updating ? 'not-allowed' : 'pointer', fontSize: '14px', marginLeft: 'auto' }}
              >
                {updating ? 'Updating...' : 'Update Ticket Tier'}
              </button>
            )}
          </div>
