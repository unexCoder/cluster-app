...
import { createEventArtistPerformanceAction } from '@/app/actions/artist-event-link'
...
interface ArtistEventLinkCreateProps {
  onNavigate: (view: string) => void
}

export default function ArtistEventLinkCreate({ onNavigate }: ArtistEventLinkCreateProps) {
  const {
    formData,
    currentStep,
    submitting,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await createEventArtistPerformanceAction(performanceData)

      if (!result.success) {
        throw new Error(result.error || 'Failed to create performance')
      }

      onNavigate('Browse Performances')
    } catch (err) {
      console.error('Error creating performance:', err)
      setValidationError('submit', err instanceof Error ? err.message : 'Failed to create performance. Please try again.')
      submitAttemptedRef.current = false
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <div className={styles.container}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '24px', fontWeight: '700' }}>Link Artist to Event</h2>
