...
// fetch union tables for font end event/venue/artist
// lib/events.ts (or wherever you want to place it)
...
export async function getEventFrontDataBySlug(slug: string) {
  ...
  AND e.deleted_at IS NULL
  AND ael.status = 'confirmed'
  ORDER BY e.start_date_time ASC, ael.performance_order ASC`, [slug]) as any[]
